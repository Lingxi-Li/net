...
TEST_CASE("uint_view") {
    SECTION("len 2") {
        ...
        SECTION("data 0xa55a") {
            ...
            SECTION("hishf 0, loshf 0") {
                net::uint_view<len> view{vec.data()};
                ...
                REQUIRE(net::uint_const_view<len>{view} == 0x5aa5);
            }
            SECTION("hishf 3, loshf 7") {
                net::uint_view<len, 3, 7> view{vec.data()};
                REQUIRE(view == view.uint());
                REQUIRE(view == 0xa);
                view = 0xffff;
    ...
    SECTION("len 1") {
        ...
        SECTION("0xa5") {
            net::byte_t data = 0xa5;
            SECTION("hishf 0, loshf 0") {
                net::uint_view<len> view{&data};
                REQUIRE(view == view.uint());
                REQUIRE(view == 0xa5);
                view = 0x5a;
                REQUIRE(data == 0x5a);
                REQUIRE(net::uint_const_view<len>{view} == 0x5a);
            }
            SECTION("hishf 2, loshf 3") {
                net::uint_view<len, 2, 3> view{&data};
                REQUIRE(view == view.uint());
                REQUIRE(view == 0x4);
                view = 0x1b;
                REQUIRE(data == 0x9d);
            ...
            SECTION("hishf 1, loshf 6") {
                net::uint_view<len, 1, 6> view{&data};
                REQUIRE(view == view.uint());
                REQUIRE(view == 0);
                view = 0xff;
                REQUIRE(data == 0xe5);
...
TEST_CASE("flags_view") {
    SECTION("len 8, hishf 0, loshf 0, mask 0x8000000000000001") {
        ...
        net::flags_view<len, 0, 0, 0x8000000000000001> flags{vec.data()};
        ...
        REQUIRE(net::flags_const_view<len, 0, 0, 0x8000000000000001>{flags} == 1);
    }
    SECTION("len 3, hishf 4, loshf 4, mask 0x300c") {
        constexpr unsigned len = 3;
        byte_vec vec(len, 0xff);
        net::flags_view<len, 4, 4, 0x300c> flags{vec.data()};
    ...
    SECTION("len 1, hishf 0, loshf 0, mask 0x55") {
        ...
        net::byte_t byte = 0xff;
        net::flags_view<len, 0, 0, 0x55> flags{&byte};
        REQUIRE(flags == 0x55);
        flags = 0;
        REQUIRE(byte == 0xaa);
        REQUIRE(flags.uint64() == 0);
        flags = 0x41;
        REQUIRE(byte == 0xeb);
        REQUIRE(net::flags_const_view<len, 0, 0, 0x55>{flags} == 0x41);
    }
}
